...
impl Compiler {
    pub fn compile_source(source: &str, options: &CompilerOptions) -> Result<Option<i32>, String> {
        ...
        let (generated_asm, has_gui) = codegen.generate(&opt_program);
        ...
        let temp_dir = std::env::temp_dir().join(format!("runvoid_{}", std::process::id()));
        ...
        // If GUI is used, compile gui.c
        if has_gui {
            fs::write(&gui_c_path, RUNTIME_GUI_C)
                .map_err(|e| format!("Failed to write gui.c: {}", e))?;

            let gcc_gui = Command::new("gcc")
                .args(["-c", "-O2"])
                .arg(&gui_c_path)
                .args(["-o"])
                .arg(&gui_obj_path)
                .output()
                .map_err(|e| format!("Failed to compile GUI runtime module with gcc: {}", e))?;
        ...
        if has_gui {
            gcc_cmd.arg(&gui_obj_path).arg("-lX11");
        }
    ...
    pub fn compile_file(file_path: &Path, options: &CompilerOptions) -> Result<Option<i32>, String> {
